  ...
  const renderCard = ({ item }) => (
    <View style={styles.card}>
      ...
      <View style={styles.cardSecondaryRow}>
        ...
        <Text style={styles.cardInfo}>{new Date(item.StartDate).toLocaleDateString('tr-TR')} {new Date(item.StartDate).toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' })}</Text>
      </View>
      <View style={styles.cardSecondaryRow}>
        <MaterialCommunityIcons name="stop-circle-outline" size={16} color="#e53935" style={{ marginRight: 7 }} />
        <Text style={styles.cardInfo}>{new Date(item.EndDate).toLocaleDateString('tr-TR')} {new Date(item.EndDate).toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' })}</Text>
      </View>
      <View style={styles.priceAndButtonsRow}>
        ...
        <View style={styles.buttonRowInline}>
          <TouchableOpacity style={styles.approveBtnSmall} onPress={() => handleApprove(item.Id)} disabled={actionLoading}>
            <MaterialIcons name="check" size={18} color="#fff" />
            <Text style={styles.btnText}>{actionLoading ? 'Onaylanıyor...' : 'Onayla'}</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.rejectBtnSmall} onPress={() => { setSelectedRequest(item.Id); setModalVisible(true); }} disabled={actionLoading}>
    ...
    <View style={styles.container}>
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<Text style={{ textAlign: 'center', marginTop: 40, color: '#888' }}>Bekleyen istek yok.</Text>}
        />
      )}
      <Modal
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: '#fff', borderRadius: 12, padding: 12, marginBottom: 12, shadowColor: '#000', shadowOpacity: 0.08, shadowRadius: 6, shadowOffset: { width: 0, height: 2 }, elevation: 2 },
  ...
  priceAndButtonsRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 8, gap: 8 },
  priceBadgeLarge: { backgroundColor: '#e8f5e9', borderRadius: 18, paddingHorizontal: 22, paddingVertical: 8, justifyContent: 'center', alignItems: 'center', marginRight: 8 },
